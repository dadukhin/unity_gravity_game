...
public class MoveBehaviour : GenericBehaviour
{
	...
	void JumpManagement()
	{
		...
		if (jump && !behaviourManager.GetAnim.GetBool(jumpBool) && behaviourManager.IsGrounded())
		{
			...
			if (behaviourManager.GetAnim.GetFloat(speedFloat) > 0)
			{
				...
				/*if (behaviourManager.GetGravityBody.GetGravityAreas.Count > 0) {
					behaviourManager.GetRigidBody.AddForce(-behaviourManager.GetGravityBody.GravityDirection * 900f, ForceMode.Impulse);
				} else {
					//behaviourManager.GetRigidBody.AddForce(Vector3.up * velocity, ForceMode.VelocityChange);
					behaviourManager.GetRigidBody.AddForce(-behaviourManager.GetGravityBody.GravityDirection * 900f, ForceMode.Impulse);

				}*/
				behaviourManager.GetRigidBody.AddForce(-behaviourManager.GetGravityBody.GravityDirection * 1200f, ForceMode.Impulse);
			}
		}
		// Is already jumping?
		else if (behaviourManager.GetAnim.GetBool(jumpBool))
		{
			//Debug.Log("jump status: " + behaviourManager.IsGrounded() + " " + isColliding + " " +  behaviourManager.GetTempLockStatus() );
			...
			//Debug.Log((Vector3.Dot(behaviourManager.GetRigidBody.velocity, -behaviourManager.GetGravityBody.GravityDirection)));
		//(Vector3.Dot(behaviourManager.GetRigidBody.velocity, -behaviourManager.GetGravityBody.GravityDirection) <= 0.1f )
			else if (behaviourManager.IsGrounded())
			{
				Debug.Log("landed");
								jump = false;
	...
	private void OnCollisionStay(Collision collision)
	{
		...
		//Debug.Log("normal" + collision.GetContact(0).normal.y + " normal: " +collision.GetContact(0).normal);
