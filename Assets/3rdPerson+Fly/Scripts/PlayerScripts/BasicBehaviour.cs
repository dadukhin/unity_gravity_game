...
public class BasicBehaviour : MonoBehaviour
{
	...
	private GravityBody gBody;                            // Reference to the player's gravity body.
	...
	// Get the player's gravity body.
	public GravityBody GetGravityBody => gBody;
	...
	void FixedUpdate()
	{
		...
		//Debug.Log("behaviors active? " + isAnyBehaviourActive + " overriding? " + overridingBehaviours.Count);
	...
	public bool IsGrounded()
	{
		...
		//return rBody.;
		//Debug.Log("isGrounded: " + this.transform.position + " col" + Physics.CheckSphere(this.transform.position, 0.01f));
		//return Physics.CheckSphere(this.transform.position, 0.01f, _groundMask);
		//(1 * Vector3.Dot(colExtents, -gBody.GravityDirection)
		//gBody.GravityDirection * (1 * colExtents.x)
		Ray ray = new Ray(GetComponent<Collider>().bounds.center, gBody.GravityDirection);
		bool test = Physics.SphereCast(ray, 0.01f, 0.94f);
		//Debug.Log("gravDir: " + gBody.GravityDirection + " transform pos " + this.transform.position + "ray: " + ray +  " collider center " +  GetComponent<Collider>().bounds.center + " is grounded: " + test);
		return test;
	}
}
